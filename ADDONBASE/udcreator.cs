    ...
    public class udcreator
    {
        ...
        public void createTablesfromXML(string xmlfile)
        {
            ...
            if (document.Element("BOM").Element("BO").Element("AdmInfo").Element("Object").Value == "153")
            {
                ...
                var pb = ADDONBASE._Initializer.SBO_Application.StatusBar.CreateProgressBar("UDTS", total, false);
        ...
        public void createUDFFromXML(string xmlfile)
        {
            ...
            if (document.Element("BOM").Element("BO").Element("AdmInfo").Element("Object").Value == "152")
            {
                ...
                var total = rows.Count();
                var count = 0;
                var pb = ADDONBASE._Initializer.SBO_Application.StatusBar.CreateProgressBar("UDFS", total, false);

                foreach (var row in rows)
                {

                    UDTCreator udtcreator = new UDTCreator(company);
                    //udtcreator.CreateUserFields(row.Element("TableName").Value.Replace("@", ""), row.Element("Name").Value, row.Element("Description").Value, SAPbobsCOM.BoFieldTypes.db_Alpha, int.Parse(row.Element("Size").Value), SAPbobsCOM.BoFldSubTypes.st_None);
                    {
                        #region Declarations
                        ...
                        if (TableName.StartsWith("@") == true)
                        {
                            ...
                            {
                                SAPbobsCOM.UserFieldsMD v_UserField = default(SAPbobsCOM.UserFieldsMD);
                                v_UserField = (SAPbobsCOM.UserFieldsMD)company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oUserFields);
                                v_UserField.TableName = TableName;
                                v_UserField.Name = FieldName;
                                v_UserField.Description = FieldDescription;
                                v_UserField.Type = Type;
                        ...
                        if (TableName.StartsWith("@") == false)
                        {
                            ...
                            if (!this.UDFExists(TableName, FieldName))
                            {
                                ...
                                {
                                    company.GetLastError(out v_ErrCode, out v_ErrMsg);
                                    Console.WriteLine("TableName = {0}, FieldName = {1}, ErrorCode = {2}, Description={3}"
        ...
        public bool createUDOFromXML(string xmlfile)
        {
            ...
            if (document.Element("BOM").Element("BO").Element("AdmInfo").Element("Object").Value == "206")
            {
                ...
                var total = rows.Count();
                var count = 0;
                var pb = ADDONBASE._Initializer.SBO_Application.StatusBar.CreateProgressBar("UDO", total, false);
                        ...
                        if (!v_ReturnCode)
                        {
                            ...
                            else
                            {
                                try
                                {
                                    ...
                                    var oRecset = company.DoQuery(query);
